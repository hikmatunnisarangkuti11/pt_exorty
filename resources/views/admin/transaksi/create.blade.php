    ...
    <div class="card">
        <div class="card-body">
           <form action="{{ route('admin.transaksi.store') }}" method="post">
            ...
            <div class="row">
                <div class="col-md-6-col-12">
                    <div class="form-group">
                        ...
                        <option value="{{ $daftars as $daftar }}" data-nama="{{ $daftar->nama }}"
                            data-harga="{{ $daftar->harga }}" data-id="{{ $daftar->id_daftar }}">
                        {{ $daftar->nama }} Rp. {{ number_format($daftar->harga) }}</option>
                        @endforeach
                    </select>
                    </div>
                </div>
            ...
            <div class="form-group">
                <label for="">&nbsp;</label>
                <button type="button" class="btn  btn-primary d-b;ock" onclick="tambahItem()">Tambah Item</button>
            </div>
        </div>
        <div class="row">
...
<script>
    ...
    function tambahItem() {
        ...
        if (item.length > 0) {
            ...
            var item = {
                id_dafter: ${'#id_daftar'}.find(':selected').data('id'),
            ...
            listItem.map((el, index) => {
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <input type="hodden" nme="quantity[]" value="${el.quantity}">
                        <button type="button" onclick="deleteItem(${index})" class="btn btn-link">
                            <i class="as fa-fw fa-trash text-danger"></i>
                            </button>
                        </td>
                    </tr>
                    `
    ...
    function updateQuantity(nom){
        quantity += nom;
        $('.quantity').html(formatRupiah(totalHarga.quantity))
    }
</script>
@endsection
